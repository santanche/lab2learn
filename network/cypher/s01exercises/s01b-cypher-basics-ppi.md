...
É possível a representação de propriedades relacionadas a um nó utilizando-se chaves. O seguinte comando cria dois nós com a propriedade `name` cujos valores são `"RAS"` e `"BRAF"`:
...
~~~cypher
CREATE (:Protein{name:"RAS"})-[:Activate]->(:Protein{name:"BRAF"})-[:Activate]->(:Protein{name:"MEK"})-[:Activate]->(:Protein{name:"ERK"})

MATCH (braf:Protein{name:"BRAF"})
CREATE (:Drug{name:"PLX4072"})-[:Inhibit]->(braf)

MATCH (ras:Protein{name:"RAS"})
MATCH (mek:Protein{name:"MEK"})
CREATE (ras)-[:Activate]->(:Protein{name:"CRAF"})-[:Activate]->(mek)
~~~
